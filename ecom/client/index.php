  ...
  <div class="best-sellers padding-top-50px">
    <div class="container">
      ...
      <div class="row">
        ...
        $total_row = $obj->count_table('productstable',$cond);
        ...
        //var_dump($total_page);die();
         if($obj->num($query1) > 0){
            ...
            {
                  ?>
                  <form method="POST" action="index.php?action=add&id=<?php echo $row['id']; ?>#anchor">
              <div class="col-md-4 col-sm-6 col-xs-12 mar-bottom-30px">
          <div class="image-wrapper"><img style="height: 100px; object-fit: cover;" 
            src="http://localhost/admin/imagesproducts/<?php echo $row['file']; ?>"  class="img-responsive"><a href="detail.php" class="text-best-seller">
...
//      var_dump($q);die();
      ...
      if($obj->num($r)> 0)
      {
        $row1 = $obj->fetch($r);
            ...
            <input type="number" name="quantity" class="btn btn-lg btn-danger "  style="float: right; width: 160px;margin-bottom: 17px;margin-left: 179px;    background-color: aliceblue; color: black;" placeholder="Quantity">
            <input type="hidden" name="hidden_category" class="btn btn-lg btn-primary " value="<?php echo $row['category']; ?>" style="float: right;">
            <input type="hidden" name="hidden_name" class="btn btn-lg btn-primary " value="<?php echo $row['name']; ?>" style="float: right;">
            <input type="hidden" name="hidden_price" class="btn btn-lg btn-primary " value="<?php echo $row['price']; ?>" style="float: right;">
            <input type="hidden" name="hidden_file" class="btn btn-lg btn-primary " value="<?php echo $row['file']; ?>" style="float: right;">
            <input type="hidden" name="hidden_category" class="btn btn-lg btn-primary " value="<?php  $q = "SELECT * FROM categoriestable WHERE id = '".$row["category_id"]."' ";
//      var_dump($q);die();
      $r = $obj->execute($q);
      if($obj->num($r)> 0)
      {
        $row1 = $obj->fetch($r);
                 ...
                 echo $row1['name']; ?>" style="float: right;">
<div class="button"><a class="btn btn-lg btn-primary " href="detail.php?id=<?php echo $row['id'];?>" role="button">EXPLORE</a><input type="submit" name="submit" class="btn btn-lg btn-primary " value="Add To Cart" style="float: right;"></div>            <hr>
